<script lang="ts">
  ...
  let eventPriority: string = "standard";
  let eventColor: string = "#3B82F6";

  // Priority to color mapping - auto-determine color from priority
  ...
  // Reactively update eventColor when eventPriority changes
  $: eventColor = getPriorityColor(eventPriority);
  ...
  function getStoredUsername(): string {
    if (browser) {
      const storedName =
        localStorage.getItem("microtask_username") ||
        "User (from Calendar Storage Default)";
  ...
  async function updateCalendarDays() {
    ...
    const daysInCurrentActualMonth = new Date(
      currentYear,
      currentMonth + 1,
      0,
    ).getDate();
    ...
    for (let i = 1; i <= daysInCurrentActualMonth; i++) {
      ...
      if (data.tasks && Array.isArray(data.tasks)) {
        data.tasks.forEach((task) => {
          if (task.dueDateISO) {
            const [taskYear, taskMonth, taskDay] = task.dueDateISO
              .split("-")
              .map(Number);
            if (
              taskYear === currentDateObj.getFullYear() &&
              taskMonth - 1 === currentDateObj.getMonth() &&
              ...
              eventsForThisDay.push({
                ...
                color: getPriorityColor(task.priority || "standard"),
  ...
  function openAddEventForm(dayDateObj?: Date) {
    ...
    eventDate = `${dateToUse.getFullYear()}-${(dateToUse.getMonth() + 1).toString().padStart(2, "0")}-${dateToUse.getDate().toString().padStart(2, "0")}`;
    eventDueTime = null;
  ...
  function openDayTasksModal(day: Day) {
    ...
    selectedDayTasks = data.tasks.filter((task) => {
      ...
      const [taskYear, taskMonth, taskDay] = task.dueDateISO
        ...
        .map(Number);
      return (
        taskYear === day.date.getFullYear() &&
        taskMonth - 1 === day.date.getMonth() &&
        taskDay === day.date.getDate()
      );
    });
  ...
  const eventFormEnhanceCallback = () => {
    ...
    return async ({
      ...
      if (result.type === "success" && result.data?.eventForm?.success) {
        closeEventForm();
        successMessage =
          result.data.eventForm.message || "Event added successfully!";
        if (successMessageTimeoutId) clearTimeout(successMessageTimeoutId);
        successMessageTimeoutId = window.setTimeout(
          () => (successMessage = null),
          3000,
        );
        await invalidateAll();
        ...
        eventFormActionError =
          failureData?.eventForm?.error || "Failed to process event.";
      } else if (result.type === "error") {
        eventFormActionError =
          (result.error as Error)?.message || "An unexpected error occurred.";
      }
      await update({
        reset: result.type === "success" && result.data?.eventForm?.success,
      });
    };
  };
  ...
  const handleEscKey = (event: KeyboardEvent) => {
    if (event.key === "Escape") {
      if (showTaskDetailsPopover) closeTaskDetailsPopover();
      else if (showEventForm) closeEventForm();
      else if (showDayTasksModal) closeDayTasksModal();
      else if (isSidebarOpen) closeSidebar();
    }
  }; // Changed to close popover
  ...
  $: {
    ...
    if (browser) {
      ...
      if (
        data.user?.name &&
        data.user.name !== "User" &&
        data.user.name !== "User (Calendar Initial Default)" &&
        data.user.name !== "User (Calendar SSR Default)" &&
        data.user.name !== "User (from Calendar Storage Default)"
      ) {
        ...
        if (
          username !== storedName &&
          (!data.user?.name || data.user.name.startsWith("User (Calendar"))
        ) {
  ...
  onMount(() => {
    ...
    if (
      data.user?.name &&
      data.user.name !== "User" &&
      !data.user.name.includes("(Calendar")
    ) {
    ...
    const handleGlobalClick = (event: MouseEvent) => {
      ...
      if (showDayTasksModal) {
        const dayTasksModalElement = document.querySelector(
          ".fixed.inset-0.z-\\[60\\]",
        ); // Assuming this class identifies the modal
    ...
    handleTaskDeletedListener = (evt: Event) => {
      try {
        console.log(
          "[Calendar] Received microtask:task-deleted — performing full reload",
        );
        ...
        console.error(
          "[Calendar] Error during task-deleted handler (reload):",
          err,
        );
      }
    };
...
>
  ...
  {#if successMessage}
    <div
      class="fixed top-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded shadow-md z-[100]"
      role="status"
    >
  ...
  {#if isSidebarOpen}
    ...
    >
      <div>
        <div
          class="flex items-center justify-between mb-8 pb-4 border-b ${isDarkMode
            ? 'border-zinc-700'
            : 'border-gray-200'}"
        >
        ...
        <nav class="flex flex-col gap-2">
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
              stroke-width="1.5"
              class={`w-5 h-5 ${currentPath === "/tasks" ? "stroke-white" : isDarkMode ? "stroke-zinc-300" : "stroke-gray-700"}`}
              ><path
                stroke-linecap="round"
                stroke-linejoin="round"
  ...
  <div class="flex-1 flex flex-col overflow-hidden pt-[60px]">
    ...
    <div class="flex-1 flex flex-col overflow-hidden h-full">
      <div class="px-4 sm:px-6 py-2 sm:py-3 flex-shrink-0">
        <h1
          class={`text-lg sm:text-xl md:text-2xl font-semibold ${isDarkMode ? "text-zinc-100" : "text-gray-800"}`}
        >
          {greeting},
        ...
        <p
          class={`text-xs sm:text-sm mt-0.5 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
        >
          Here is your monthly schedule.
        </p>
      </div>

      <main
        class="px-4 sm:px-6 lg:px-6 xl:px-20 2xl:px-30 pb-4 flex-1 flex flex-col min-h-0 overflow-hidden"
      >
        <div
          class={`calendar-container w-full max-w-7xl mx-auto p-2 sm:p-3 rounded-lg shadow flex flex-col flex-1 min-h-0 overflow-hidden ${isDarkMode ? "bg-zinc-800 border border-zinc-700" : "bg-white border border-gray-200"}`}
        >
          <div
            class="flex justify-between items-center mb-2 sm:mb-3 px-1 flex-shrink-0"
          >
            <button
              on:click={prevMonth}
              aria-label="Previous Month"
              class={`p-2 rounded-md transition-colors ${isDarkMode ? "hover:bg-zinc-700 text-zinc-300" : "hover:bg-gray-100 text-gray-600"}`}
              ><svg
                class="w-5 h-5"
                fill="none"
                stroke="currentColor"
            ...
            >
            <h2
              class={`text-lg sm:text-xl font-semibold ${isDarkMode ? "text-zinc-100" : "text-gray-800"}`}
            >
          ...
          <div
            class={`grid grid-cols-7 gap-px text-center font-medium text-[0.65rem] sm:text-xs mb-1 flex-shrink-0 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
          >
            {#each ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"] as dayName}<div
                class="py-0.5 sm:py-1 hidden sm:block"
              >
                {dayName.toUpperCase()}
              </div>
              <div class="py-0.5 sm:py-1 sm:hidden">{dayName[0]}</div>{/each}
          </div>
          <div class="grid grid-cols-7 gap-px flex-1 min-h-0 auto-rows-fr">
            {#if isLoadingCalendar && daysInMonth.length === 0}
              <div
                class="col-span-7 text-center py-10 flex justify-center items-center min-h-[300px]"
              >
                <svg
                  class={`animate-spin h-8 w-8 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  ><circle
            ...
            {:else if daysInMonth.length > 0}
              {#each daysInMonth as day (day.date.toISOString() + "-" + day.dayNum + "-" + day.events
                  .map((e) => e.id + (e.deadlineTime || ""))
                  .join("-"))}
                <div
                  class={`p-0.5 sm:p-1 border flex flex-col transition-colors duration-150 cursor-pointer relative group overflow-hidden ${!day.isCurrentMonth ? (isDarkMode ? "bg-zinc-850 border-zinc-750 text-zinc-600" : "bg-gray-50 border-gray-100 text-gray-400") : isDarkMode ? "bg-zinc-750 border-zinc-700 hover:bg-zinc-700" : "bg-white border-gray-200 hover:bg-gray-50"} ${day.isToday && day.isCurrentMonth ? (isDarkMode ? "!border-blue-500 ring-1 ring-blue-500 !bg-zinc-700" : "!border-blue-500 ring-1 ring-blue-500 !bg-blue-50") : ""}`}
                  role="gridcell"
                  aria-label={`Date ${day.date.toLocaleDateString()}${day.events.length ? ", " + day.events.length + " event" + (day.events.length > 1 ? "s" : "") : ""}`}
                  on:click={() => openDayTasksModal(day)}
                  tabindex="0"
                  on:keydown={(e) => {
                ...
                >
                  <span
                    class={`text-[0.65rem] sm:text-xs md:text-sm font-semibold flex-shrink-0 ${day.isToday && day.isCurrentMonth ? (isDarkMode ? "text-blue-300" : "text-blue-600") : ""} ${!day.isCurrentMonth ? "opacity-60" : ""}`}
                    >{day.dayNum}</span
                  >
                  {#if day.isCurrentMonth && day.events.length > 0}
                    {@const maxEventsToShow = day.events.length > 2 ? 1 : 2}
                    <ul
                      class="mt-0.5 text-[0.5rem] sm:text-[0.6rem] md:text-[0.7rem] space-y-0.5 overflow-hidden flex-1 min-h-0"
                    >
                      {#each day.events.slice(0, maxEventsToShow) as event (event.id)}
                        <li
                          class={`truncate px-0.5 py-px sm:px-1 sm:py-0.5 rounded text-white leading-tight cursor-pointer hover:opacity-80 transition-opacity`}
                          style="background-color: {event.color ||
                            (isDarkMode ? '#374151' : '#9CA3AF')};"
                          on:mouseenter={(e) =>
                            openTaskDetailsPopover(event.id, e)}
                          on:mouseleave={scheduleClosePopover}
                          on:keydown|stopPropagation={(e) => {
                            if (e.key === "Enter" || e.key === " ")
                              openTaskDetailsPopover(event.id);
                          }}
                          tabindex="0"
                          role="button"
                        ...
                        >
                          {event.title}
                          {#if event.deadlineTime}<span
                              class="hidden sm:block text-[0.5rem] sm:text-[0.55rem] opacity-80"
                              >DL: {event.deadlineTime}</span
                            >{/if}
                        </li>
                      {/each}
                      {#if day.events.length > maxEventsToShow}
                        {@const hiddenEvents =
                          day.events.slice(maxEventsToShow)}
                          ...
                          <div class="flex items-center justify-between gap-1">
                            <div class="flex items-center gap-0.5">
                              {#each hiddenEvents.slice(0, 5) as hEvent}
                                ...
                                <span
                                  class={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full ${dotColor} shadow-sm ring-1 ring-white/30`}
                                ></span>
                              {/each}
                              {#if hiddenCount > 5}
                                <span
                                  class={`text-[0.5rem] ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
                                  >...</span
                                >
                              {/if}
                            </div>
                            <span
                              class={`text-[0.55rem] sm:text-[0.65rem] font-bold px-1.5 py-0.5 rounded-full ${isDarkMode ? "bg-purple-600 text-white" : "bg-purple-500 text-white"}`}
                            >
                              +{hiddenCount}
                            </span>
                          </div>
                  ...
                  <button
                    ...
                    class={`absolute bottom-0.5 right-0.5 sm:bottom-1 sm:right-1 p-0.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-150 focus:opacity-100 ${isDarkMode ? "bg-blue-600 hover:bg-blue-500 text-white" : "bg-blue-500 hover:bg-blue-600 text-white"}`}
                    aria-label="Add event to this day"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      class="w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3"
                      ><path
            ...
            {:else if !isLoadingCalendar && daysInMonth.length === 0}
              <p
                class={`col-span-7 text-center py-10 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
              >
    ...
    on:delete={async (event) => {
      ...
      try {
        ...
        if (parsedData.deleteTaskForm?.error) {
          console.error(
            `Error deleting task: ${parsedData.deleteTaskForm.error}`,
          );
        }
        await invalidateAll();
      } catch (error) {
  ...
  <CalendarAIPanel
    ...
    on:selectTask={(e) => {
      // Open task detail modal to show task details
      if (e.detail.task) {
        openTaskDetailModal(e.detail.task);
      }
    }}
...
<style>
  ...
  :global(.dark .bg-zinc-850) {
    background-color: #1a202c;
  } /* Slightly darker for non-current month days */
  :global(.dark .border-zinc-750) {
    border-color: #2d3748;
  } /* Border for non-current month days */
  :global(.dark .bg-zinc-750) {
    background-color: #2d3748;
  } /* Bg for current month days in dark */

  :global(.dark .calendar-picker-dark::-webkit-calendar-picker-indicator) {
    filter: invert(
      0.8
    ); /* Makes the date/time picker icon visible in dark mode */
  }
