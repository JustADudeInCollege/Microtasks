<script lang="ts">
    ...
    function handleLogout() {
        console.log(
            "[Workspace] handleLogout called, showing confirmation modal",
        );
    ...
    function toggleWindow(id: string) {
        ...
        if (el) {
            ...
            if (isHidden) {
                // If it *was* hidden, and now it's being shown
            ...
            steps: [
                {
                    ...
                    inputPrompt:
                        'List the main subjects or topics you need to cover for your exams (e.g., "Calculus II, Organic Chemistry - Chapters 1-5, World History 1800-1900")?',
                    inputType: "textarea",
                },
                {
            ...
            steps: [
                {
                    ...
                    inputPrompt:
                        "What is the overall scope and what are the main objectives of this project?",
                    inputType: "textarea",
                },
                {
                    text: "Identify key stakeholders and their roles",
                    inputPrompt:
                        'List key stakeholders and their primary roles/responsibilities (e.g., "Jane Doe - Product Owner, Marketing Team - Feedback & Promotion")?',
                    inputType: "textarea",
                },
                {
            ...
            steps: [
                {
                    ...
                    inputPrompt:
                        'What specific habit or skill are you aiming to develop (e.g., "Meditate for 10 minutes daily", "Learn conversational Spanish")?',
                    inputType: "text",
                },
                {
                ...
                },
                {
                    ...
                    inputPrompt:
                        'What is your Specific, Measurable, Achievable, Relevant, and Time-bound (SMART) goal (e.g., "Meditate 5 days a week for 1 month", "Be able to hold a 5-minute basic conversation in Spanish in 3 months")?',
                    inputType: "textarea",
                },
    ...
    let pdfFiles: {
        name: string;
        status: "reading" | "scanning" | "ready" | "error";
        errorMessage?: string;
    }[] = [];
    ...
    async function handlePdfUpload(event: Event) {
        ...
        pdfFiles = Array.from(input.files).map((f) => ({
            name: f.name,
            status: "reading",
        }));
        pdfContextText = "";
        ...
        try {
            ...
            for (let i = 0; i < input.files.length; i++) {
                ...
                try {
                    ...
                    if (cleanText.length < 20) {
                        console.log(
                            `File ${file.name} has little text (${cleanText.length} chars). Attempting OCR...`,
                        );
                        ...
                        try {
                            ...
                            if (cleanOcrText.length > 20) {
                                console.log(
                                    `OCR successful for ${file.name}. Found ${cleanOcrText.length} chars.`,
                                );
                                cleanText = cleanOcrText; // Replace the empty text with OCR text!
                            } else {
                                console.warn(
                                    `OCR also failed/empty for ${file.name}. Raw OCR length: ${ocrText.length}`,
                                );
                            }
                        } catch (ocrErr: any) {
                            console.error(
                            ...
                            pdfFiles[i] = {
                                ...pdfFiles[i],
                                errorMessage: `OCR Error: ${ocrErr.message || ocrErr}`,
                            };
                        }
                    }
                    ...
                    if (cleanText.length > 20) {
                        ...
                        console.warn(
                            `File ${file.name} extracted to empty/short text. Clean length: ${cleanText.length}`,
                        );
                        ...
                        if (!pdfFiles[i].errorMessage) {
                            pdfFiles[i] = {
                                ...
                                status: "error",
                                errorMessage:
                                    "No text found. OCR failed or file is empty.",
                            };
                        } else {
            ...
            console.log(
                "All PDFs parsed. Total length:",
                pdfContextText.length,
            );
        } catch (err) {
        ...
        if (form) {
            ...
            if (form.boardForm) {
                ...
                if ("error" in form.boardForm) {
                    ...
                    if (
                        "title" in form.boardForm &&
                        typeof form.boardForm.title === "string"
                    ) {
    ...
    onMount(() => {
        ...
        handleGlobalClickListener = (event: MouseEvent) => {
            ...
            const modals = [
                {
                    modalId: "addBoardModalContent",
                    condition: showAddBoardModal,
                    closeFn: closeAddBoardModal,
                },
                {
                    modalId: "deleteBoardModalContent",
                    condition: showDeleteBoardConfirm,
                    closeFn: closeDeleteBoardConfirm,
                },
                {
                    modalId: "templateUsageModalContent",
                    condition: showTemplateUsageModal,
                    closeFn: closeTemplateUsageModal,
                },
            ];
            modals.forEach((modal) => {
                ...
                if (
                    modal.condition &&
                    modalEl &&
                    !modalEl.contains(target as Node)
                ) {
                    const overlayEl = modalEl.parentElement;
                    if (
                        overlayEl &&
                        overlayEl.classList.contains("modal-overlay") &&
                        overlayEl === event.target
                    ) {
                        modal.closeFn();
                    }
                }
...
<div class="app-layout">
    {#if isSidebarOpen}
        ...
        >
                ...
                <nav class="flex flex-col gap-2">
                    ...
                    >
                        <svg
                            ...
                            stroke-width="1.5"
                            stroke="currentColor"
                            class="w-5 h-5 nav-link-icon-stroked"
                            aria-hidden="true"
                        >
                            <path
                                stroke-linecap="round"
            ...
            <button
                ...
                class="logout-button flex items-center gap-3 px-3 py-2 rounded-md font-semibold w-full mt-auto transition-colors duration-150"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    ...
                    stroke-width="1.5"
                    stroke="currentColor"
                    class="w-5 h-5 logout-button-icon"
                    aria-hidden="true"
                    ><path
                        stroke-linecap="round"
                        stroke-linejoin="round"
    ...
    <div class="main-content-wrapper">
        ...
        {#if pageSuccessMessage}
            <div class="alert alert-success" role="alert">
                ...
                <button
                    on:click={() => (pageSuccessMessage = null)}
                    class="alert-close-button"
        ...
        <main class="main-area">
            <div class="container">
                ...
                <section class="workspace-section">
                    <h3 class="sub-section-title sr-only">
                            ...
                            {#each boards as board (board.id)}
                                ...
                                >
                                    ...
                                    <div class="board-card-info">
                                        ...
                                        {#if !board.isOwner}
                                            <span
                                                class="role-badge {board.userRole}"
                                                >{board.userRole}</span
                                            >
                                        {/if}
                                    </div>
                    ...
                    {:else if !data.error}
                        <div class="empty-state-workspaces">
                            ...
                            <p>
                                ...
                                workspaces. <br />
                                Workspaces help you organize your tasks into projects
                                or different areas of focus.
                            </p>
                            <button
                                on:click={openAddBoardModal}
                ...
                <section class="template-section">
                    ...
                    <p class="section-description">
                        Having a hard time getting started? Use our ai assisted
                        workspace templates to jump right in and stay
                        productive—no stress, just progress.
                    </p>
                        ...
                        {#each templates as template (template.title)}
                            <div class="template-card">
                                    ...
                                    <span class="template-card-icon-wrapper">
                                        {#if template.iconKey === "study"}
                                            <svg
                                                ...
                                                fill="currentColor"
                                                ><path
                                                    d="M2.5 1A1.5 1.5 0 0 0 1 2.5v15A1.5 1.5 0 0 0 2.5 19h15a1.5 1.5 0 0 0 1.5-1.5v-15A1.5 1.5 0 0 0 17.5 1h-15ZM2.75 5.24v11.261c0 .136.11.25.25.25h13.998a.25.25 0 0 0 .251-.25V5.239c-.54.091-1.12.15-1.749.17P12.5 5.5 10 6l-2.5-.5c-.63-.02-1.209-.079-1.751-.17Z"
                                                /></svg
                                            >
                                        {:else if template.iconKey === "work"}
                                            <svg
                                                ...
                                                ><path
                                                    fill-rule="evenodd"
                                                    d="M6 3.75A2.75 2.75 0 0 1 8.75 1h2.5A2.75 2.75 0 0 1 14 3.75v.438A2.5 2.5 0 0 0 12.5 3h-5A2.5 2.5 0 0 0 6 4.188V3.75ZM13.5 6A1.5 1.5 0 0 1 15 7.5v6A1.5 1.5 0 0 1 13.5 15h-7A1.5 1.5 0 0 1 5 13.5v-6A1.5 1.5 0 0 1 6.5 6h7Zm0 1.5H6.5V13h7V7.5Z"
                                                    clip-rule="evenodd"
                                                /></svg
                                            >
                                        {:else if template.iconKey === "self-improvement"}
                                            <svg
                                                ...
                                                fill="currentColor"
                                                ><path
                                                    d="M11.213 4.042S8.446 3.214 6.83 4.83c-1.224 1.223-1.037 3.304.196 4.537l-.392.393a.75.75 0 001.06 1.061l.393-.392c1.233 1.233 3.314 1.037 4.537.196 1.617-1.617.789-4.384.789-4.384S12.436 5.266 11.213 4.042ZM10 9.5a1.5 1.5 0 110-3 1.5 1.5 0 010 3Z"
                                                /><path
                                                    d="M17.5 6.5c0 .69-.56 1.25-1.25 1.25S15 7.19 15 6.5s.56-1.25 1.25-1.25S17.5 5.81 17.5 6.5ZM14.452 2.456a1.25 1.25 0 011.768 0l.022.021c.48.48.48 1.256 0 1.736L15.25 5.206a1.25 1.25 0 01-1.736-1.736l.938-.914Z"
                                                /></svg
                                            >
                                        {:else}
                                            <!-- Default icon -->
                                ...
                                <p class="template-card-goal">
                                    <strong class="font-medium">Goal:</strong>
                                    {template.goal}
                                </p>
                                ...
                                <button
                                    on:click={() =>
                                        openTemplateUsageModal(template)}
            ...
            <div id="addBoardModalContent" class="modal-content">
                ...
                >
                    <div class="modal-form-group">
                        <label for="workspaceNameModalInput" class="modal-label"
                            >Workspace Name <span class="required-asterisk"
                                >*</span
                            ></label
                        >
                        <input
                            type="text"
                            id="workspaceNameModalInput"
                            name="title"
    ...
    {#if showTemplateUsageModal && selectedTemplateForUsage}
        <div class="modal-overlay" id="templateUsageModalOverlay">
            <div
                id="templateUsageModalContent"
                class="modal-content modal-content-large"
            >
                <div class="modal-header">
                    <h3 class="modal-title">
                        Use Template: {selectedTemplateForUsage.title}
                    </h3>
                    <button
                        on:click={closeTemplateUsageModal}
                        class="modal-close-button"
                    >
                        <span>×</span>
                    </button>
                ...
                <form
                    method="POST"
                    action="?/createWorkspaceFromTemplate"
                    use:enhance={() => {
                ...
                >
                    <div class="modal-form-scrollable custom-scrollbar">
                        <p class="template-modal-goal">
                            <strong class="font-medium">Template Goal:</strong>
                            {selectedTemplateForUsage.goal}
                        </p>

                        <input
                            type="hidden"
                            name="templateTitle"
                            value={selectedTemplateForUsage.title}
                        />
                        <input
                            type="hidden"
                            name="templateGoal"
                            value={selectedTemplateForUsage.goal}
                        />
                        <input
                            type="hidden"
                            name="templateStepsJSON"
                            value={JSON.stringify(
                                selectedTemplateForUsage.steps,
                            )}
                        />
                        <input
                            type="hidden"
                            name="stepSpecificInputsJSON"
                            value={JSON.stringify(stepSpecificInputs)}
                        />

                        <div class="modal-form-group">
                            <label
                                for="newWorkspaceNameFromTemplate"
                                class="modal-label"
                                >Name for your new workspace <span
                                    class="required-asterisk">*</span
                                ></label
                            >
                            <input
                                type="text"
                                id="newWorkspaceNameFromTemplate"
                                name="workspaceName"
                                bind:value={newWorkspaceNameFromTemplate}
                                required
                                maxlength="100"
                                class="modal-input"
                        ...
                        <div class="date-input-grid">
                            <div>
                                ...
                                />
                                <p class="input-hint">
                                    Tasks will be scheduled from this date.
                                </p>
                            </div>
                            <div>
                                <label class="modal-label"
                                    >Plan End Date (Optional)</label
                                >
                                <DatePicker
                                ...
                                />
                                <p class="input-hint">
                                    Distribute tasks until this date if set.
                                </p>
                            </div>
                        </div>

                        <hr class="modal-divider" />
                        <p class="ai-prompt-section-title">
                            Help the AI tailor your tasks:
                        </p>
                        ...
                        {#each selectedTemplateForUsage.steps as step, i}
                            {#if step.inputPrompt}
                                <div class="step-input-container">
                                    <label
                                        for={`step-input-${i}`}
                                        class="step-input-label"
                                        >{step.inputPrompt}</label
                                    >
                                    {#if step.inputType === "textarea"}
                                        <textarea
                                            id={`step-input-${i}`}
                                            name={`step_input_${i}`}
                                            bind:value={stepSpecificInputs[i]}
                                            rows="2"
                                    ...
                                    {:else}
                                        ...
                                        />
                                    {/if}
                                    <p class="step-input-hint">
                                        AI will use this to customize the task:
                                        "<em>{step.text}</em>".
                                    </p>
                                </div>
                            {/if}
                        ...
                        <div class="modal-form-group">
                            <label for="projectNotes" class="modal-label"
                                >General Notes/Additional Context (Optional)</label
                            >
                            <textarea
                                id="projectNotes"
                                name="projectNotes"
                        ...
                        {#if templateFormActionError}
                            <p class="form-action-error">
                                {templateFormActionError}
                            </p>
                        {/if}
                        ...
                        <div class="modal-form-group">
                            <label class="modal-label"
                                >Upload Reference PDFs (Optional)</label
                            >
                            <div class="pdf-upload-container">
                                ...
                                <label
                                    for="pdfUpload"
                                    class="button button-secondary pdf-upload-btn"
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                ...
                                <div class="pdf-file-list">
                                    {#each pdfFiles as file}
                                        <div class="pdf-file-info">
                                            ...
                                            >
                                            <span
                                                class="parse-status {file.status}"
                            ...
                            <p class="input-hint">
                                Upload relevant documents (syllabi, briefs,
                    ...
                    <div class="modal-footer-actions">
                        ...
                        <button
                            ...
                            class="button button-success"
                            disabled={isSubmittingTemplateWorkspace ||
                                !newWorkspaceNameFromTemplate.trim()}
                        >
...
<style>
    :root {
        ...
        --light-accent-primary-text: var(
            --light-text-link
        ); /* For text links or icons needing this blue */
        ...
        --shadow-md: 0 3px 6px -1px rgba(0, 0, 0, 0.05),
            0 2px 4px -1px rgba(0, 0, 0, 0.03);
        --shadow-lg: 0 8px 15px -3px rgba(0, 0, 0, 0.07),
            0 4px 6px -2px rgba(0, 0, 0, 0.04);
        --shadow-xl: 0 15px 25px -5px rgba(0, 0, 0, 0.08),
        ...
        --dark-shadow-md: 0 3px 6px -1px rgba(0, 0, 0, 0.15),
            0 2px 4px -1px rgba(0, 0, 0, 0.1);
        --dark-shadow-lg: 0 8px 15px -3px rgba(0, 0, 0, 0.2),
            0 4px 6px -2px rgba(0, 0, 0, 0.15);
        --dark-shadow-xl: 0 15px 25px -5px rgba(0, 0, 0, 0.25),
    ...
    .custom-scrollbar {
        ...
        scrollbar-color: #bdc1c5 var(--light-bg-app);
    }

    :global(body.dark ::-webkit-scrollbar-track) {
        background: var(--dark-bg-app);
    }
    :global(body.dark ::-webkit-scrollbar-thumb) {
        background: #4f5b64;
    }
    :global(body.dark ::-webkit-scrollbar-thumb:hover) {
        background: #606c76;
    }
    :global(body.dark) .custom-scrollbar {
        scrollbar-color: #4f5b64 var(--dark-bg-app);
    }
    ...
    :global(body.dark) .sub-section-title {
    ...
    /* SIDEBAR */
    #sidebar {
        /* Using ID selector to match HTML */
    ...
    /* Sidebar Logo Img */
    /* Tailwind: w-8 h-8 */
    #sidebar img[alt="Microtask Logo"] {
        width: 2rem;
        height: 2rem;
    }
    /* Sidebar Title H1 */
    ...
    .nav-link {
        /* Added this class to <a> tags in HTML */
        display: flex;
        align-items: center;
        gap: var(--space-3);
    ...
    .nav-link .nav-link-icon-stroked {
        width: 1.25rem;
        height: 1.25rem;
    ...
    .nav-link.active .nav-link-icon {
        fill: var(--light-text-on-accent);
    }
    .nav-link.active .nav-link-icon-stroked {
        stroke: var(--light-text-on-accent);
        fill: none;
    }
    ...
    .logout-button {
        /* Added this class to button in HTML */
        display: flex;
        align-items: center;
        gap: var(--space-3);
    ...
    :global(body.dark) .board-card,
    :global(body.dark) .template-card {
        background-color: var(--dark-bg-content);
        box-shadow: var(--dark-shadow-md);
    }
    :global(body.dark) .board-card:hover,
    :global(body.dark) .template-card:hover {
        box-shadow: var(--dark-shadow-lg);
    }
    ...
    .board-card-header {
        ...
        padding-right: 2rem; /* Make room for the absolute-positioned delete button */
    }
    .board-card-title {
        font-size: 1.25rem;
        font-weight: 600;
    ...
    :global(body.dark) .board-card-title {
        color: var(--dark-accent-primary-text);
    }
    .board-card-meta {
        font-size: 0.8rem;
        color: var(--light-text-tertiary);
    }
    :global(body.dark) .board-card-meta {
        color: var(--dark-text-tertiary);
    }
    ...
    .board-card-delete-btn:disabled {
        opacity: 0.5;
        cursor: not-allowed;
    }
    :global(body.dark) .board-card-delete-btn {
        color: var(--dark-text-tertiary);
    }
    ...
    .board-card-delete-btn .icon-sm,
    .board-card-delete-btn .spinner {
        width: 1rem;
        height: 1rem;
    ...
    .add-workspace-card .icon-lg {
        width: 2.5rem;
        height: 2.5rem;
        margin-bottom: var(--space-3);
        fill: currentColor;
    }
    .add-workspace-card .text-sm {
        font-size: 0.9rem;
    }
    .add-workspace-card .font-medium {
        font-weight: 500;
    }
    ...
    :global(body.dark) .empty-state-workspaces {
        background-color: var(--dark-bg-content);
        box-shadow: var(--dark-shadow-md);
    }
    ...
    :global(body.dark) .empty-state-workspaces .icon-xl {
        color: var(--dark-text-tertiary);
    }
    ...
    :global(body.dark) .empty-state-workspaces h3 {
        color: var(--dark-text-primary);
    }
    .empty-state-workspaces p {
        color: var(--light-text-secondary);
        margin-bottom: var(--space-6);
        line-height: 1.7;
    }
    :global(body.dark) .empty-state-workspaces p {
        color: var(--dark-text-secondary);
    }
    ...
    .template-card-icon-wrapper svg {
        width: 1.5rem;
        height: 1.5rem;
        color: var(--light-accent-info-icon);
        fill: currentColor;
    }
    :global(body.dark) .template-card-icon-wrapper {
        background-color: var(--dark-accent-info-bg);
    }
    :global(body.dark) .template-card-icon-wrapper svg {
        color: var(--dark-accent-info-icon);
    }
    .template-card-title {
        font-size: 1.25rem;
        font-weight: 600;
    ...
    :global(body.dark) .template-card-title {
        color: var(--dark-text-primary);
    }
    .template-card-goal {
        font-size: 0.9rem;
    ...
    .template-card-goal strong {
        font-weight: 500;
        color: var(--light-text-primary);
    }
    :global(body.dark) .template-card-goal {
        color: var(--dark-text-secondary);
    }
    :global(body.dark) .template-card-goal strong {
        color: var(--dark-text-primary);
    }
    .template-card-steps {
    ...
    .template-card-steps li {
        margin-bottom: var(--space-1_5);
    }
    :global(body.dark) .template-card-steps {
        color: var(--dark-text-tertiary);
    }
    ...
    /* MODALS - Modern Glassmorphism Style */
    .modal-overlay {
        position: fixed;
        inset: 0;
        z-index: 100;
    ...
    :global(body.dark) .modal-overlay {
    ...
    .modal-title-danger {
        color: var(--light-accent-danger);
    }
    :global(body.dark) .modal-title {
        color: var(--dark-text-primary);
    }
    :global(body.dark) .modal-title-danger {
    ...
    .modal-close-button span {
        ...
        width: 100%;
        height: 100%;
        margin-top: -2px; /* Fine-tune vertical alignment for × character */
    }
    ...
    :global(body.dark) .modal-input:focus,
    :global(body.dark) .modal-textarea:focus {
        border-color: var(--dark-accent-primary);
    ...
    :global(body.dark) .form-action-error {
        color: var(--dark-accent-danger-text);
        background-color: var(--dark-accent-danger-bg);
        border-color: var(--dark-accent-danger-border);
    }
    .form-success-message {
        color: var(--light-accent-success-text);
        background-color: var(--light-accent-success-bg);
        border: 1px solid var(--light-accent-success-border);
    }
    :global(body.dark) .form-success-message {
        color: var(--dark-accent-success-text);
        background-color: var(--dark-accent-success-bg);
        border-color: var(--dark-accent-success-border);
    }
    ...
    :global(body.dark) .button-default:hover:not(:disabled) {
    ...
    .button-primary:focus-visible {
        outline: none;
        box-shadow: var(--shadow-sm), var(--shadow-focus-ring);
    }
    :global(body.dark) .button-primary {
        background: linear-gradient(135deg, #3b82f6, #2563eb);
    }
    :global(body.dark) .button-primary:hover:not(:disabled) {
        background: linear-gradient(135deg, #2563eb, #1d4ed8);
        box-shadow: 0 4px 12px rgba(59, 130, 246, 0.4);
    }
    :global(body.dark) .button-primary:focus-visible {
        box-shadow: var(--dark-shadow-sm), var(--dark-shadow-focus-ring);
    }

    .button-danger {
        color: var(--light-text-on-accent);
        background-color: var(--light-accent-danger);
    }
    .button-danger:hover:not(:disabled) {
        background-color: var(--light-accent-danger-hover);
    }
    ...
    :global(body.dark) .button-danger {
        background-color: var(--dark-accent-danger);
    }
    :global(body.dark) .button-danger:hover:not(:disabled) {
        background-color: var(--dark-accent-danger-hover);
    }
    :global(body.dark) .button-danger:focus-visible {
        box-shadow:
            var(--dark-shadow-sm),
    ...
    :global(body.dark) .button-success {
        background: linear-gradient(135deg, #22c55e, #16a34a);
    }
    :global(body.dark) .button-success:hover:not(:disabled) {
        background: linear-gradient(135deg, #16a34a, #15803d);
        box-shadow: 0 4px 12px rgba(34, 197, 94, 0.4);
    }
    :global(body.dark) .button-success:focus-visible {
        box-shadow:
            var(--dark-shadow-sm),
    ...
    :global(body.dark) .modal-text-confirm {
        color: var(--dark-text-secondary);
    }
    ...
    :global(body.dark) .modal-text-secondary {
        color: var(--dark-text-tertiary);
    }
    .modal-text-warning {
        font-size: 0.9rem;
    ...
    .template-modal-goal strong {
        font-weight: 500;
        color: var(--light-text-primary);
    }
    :global(body.dark) .template-modal-goal {
        color: var(--dark-text-secondary);
    }
    :global(body.dark) .template-modal-goal strong {
        color: var(--dark-text-primary);
    }
    .date-input-grid {
        display: grid;
    ...
    :global(body.dark) .modal-divider {
        border-top-color: var(--dark-border-primary);
    }
    ...
    :global(body.dark) .ai-prompt-section-title {
        color: var(--dark-text-secondary);
    }
    ...
    :global(body.dark) .step-input-label {
        color: var(--dark-text-secondary);
    }
    .step-input-hint {
        font-size: 0.8rem;
    ...
    :global(body.dark) .step-input-hint {
        color: var(--dark-text-tertiary);
    }
    :global(body.dark) .step-input-hint em {
        color: var(--dark-text-primary);
    }
    ...
    :global(body.dark) .modal-footer-actions {
        border-top-color: var(--dark-border-primary);
    }
